...
module.exports.postOneOrderProduct = ({order_quantity, order_id, product_id}) => {
    return new Promise((resolve, reject) => {
        module.exports.getLastOrderProduct().then(lineID => {
            let id = lineID + 1;
            db.run(`INSERT into order_product VALUES(${id}, ${order_quantity}, ${order_id}, ${product_id})`, function(err, order) {
                if (err) return reject(err);
